        ...
        match Kind::from(addr.kind) {
            Kind::Ipv4 => {
                if addr.address.len() != 4 {
                    debug!("Invalid IPv4 address length: {}", addr.address.len());
            ...
            Kind::Ipv6 => {
                if addr.address.len() != 16 {
                    debug!("Invalid IPv6 address length: {}", addr.address.len());
            ...
            Kind::DomainName => {
                if addr.address.is_empty() {
                    debug!("Invalid domain name address length: {}", addr.address.len());

                    return None;
                }

                if addr.address.len() < 2 {
                    debug!("Domain name address is too short: {}", addr.address.len());
...
impl From<&[u8]> for Request {
    fn from(buffer: &[u8]) -> Self {
        ...
        dbg!(String::from_utf8_lossy(&buffer[3..addr_end_position + 1]));
